...
Tinytest.add('livedata - crossbar', function (test) {
  ...
  test.isTrue(
    crossbar._matches({ collection: 'C', id: 'X' }, { collection: 'C' })
  );
  test.isTrue(
    crossbar._matches({ collection: 'C' }, { collection: 'C', id: 'X' })
  );
  test.isTrue(
    crossbar._matches({ collection: 'C', id: 'X' }, { collection: 'C' })
  );
  ...
  crossbar.listen(trigger, function (notification) {
    ...
    if (calledSecond) {
      test.fail({
        ...
        message:
          'test assumed that listeners would be called in the order registered',
      });
    } else {
      secondHandle.stop();
    }
