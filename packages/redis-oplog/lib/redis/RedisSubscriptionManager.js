...
class RedisSubscriptionManager {
    ...
    process(channel, data, fromRedis) {
        advancedDebug({log: 'RedisSubscriptionManager.process.start', docId: data?.d?._id, channel, data, fromRedis });
        ...
        if (!subscribers) {
            advancedDebug({log: 'subscriber not found ', docId: data?.d?._id, channel });
            return;
        }
        ...
        advancedDebug({log: 'subscribers', docId: data?.d?._id, channel, length: subscribers.length});
        ...
        if (!isSynthetic) {
            ...
            subscribers.forEach(redisSubscriber => {
                try {
                    advancedDebug({log: 'processSynthetic', docId: data?.d?._id, channel, data });
    ...
    getDoc(collection, subscribers, data) {
        ...
        if (collection._redisOplog && !collection._redisOplog.protectAgainstRaceConditions) {
            ...
            advancedDebug({log: 'getDoc, no protection against race conditions', doc, docId: doc._id });
            return doc;
        }

        const fieldsOfInterest = getFieldsOfInterestFromAll(subscribers);

        advancedDebug({log: 'fieldsOfInterest', doc, docId: doc._id, fieldsOfInterest });
