...
export default class Mutator {
    ...
    static update(Originals, selector, modifier, _config, callback) {
        advancedDebug({log: 'Redis mutator update', collectionName: this._name, selectorId: selector?._id, selector, modifier})
        ...
        const findOptions = { fields: { _id: 1 }, transform: null, noSecurity: true };
        if (!config.multi) {
            findOptions.limit = 1;
        }
