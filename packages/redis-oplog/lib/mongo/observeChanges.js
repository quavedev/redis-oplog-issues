...
export default function(cursorDescription, ordered, callbacks) {
    ...
    const fields = cursorDescription.options.projection || cursorDescription.options.fields;

    if (
        fields &&
        (fields._id === 0 || fields._id === false)
    ) {
    ...
    advancedDebug({log: 'add observeChanges observeKey', cursorDescription, observeKey });
    ...
    advancedDebug({log: 'add observeChanges firstHandle', cursorDescription, firstHandle });
            ...
            [
                ...
                function() {
                    ...
                    try {
                        ...
                        advancedDebug({log: 'canUseOplog error on matcher', cursorDescription });
                ...
                function() {
                    ...
                    try {
                        ...
                        // XXX make all compilation errors MinimongoError or something
                        //     so that this doesn't ignore unrelated exceptions
                        advancedDebug({log: 'canUseOplog error on matcher with sorter', cursorDescription });
                        return false;
                    }
                },
            ],
    ...
    advancedDebug({log: 'add observeChanges before addHandleAndSendInitialAdds' });
    multiplexer.addHandleAndSendInitialAdds(observeHandle);
    advancedDebug({log: 'add observeChanges after addHandleAndSendInitialAdds' });


    return observeHandle;
} catch(e) {
    advancedDebug({log: 'error in observeChanges', cursorDescription, error: e });
}
}
