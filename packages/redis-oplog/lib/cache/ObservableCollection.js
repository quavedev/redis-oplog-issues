...
export default class ObservableCollection {
    ...
    isEligible(doc) {
        if (this.matcher) {
            advancedDebug({log: 'isEligible', doc, docId: doc?._id, matcher: this.matcher });
    ...
    change(doc, modifiedFields) {
        const docId = doc._id;
        advancedDebug({log: 'ObservableCollection change', doc, docId: doc?._id, modifiedFields});
        const oldDoc = this.store.get(docId);
        if (oldDoc == null) {
            advancedDebug({log: 'ObservableCollection change, no oldDoc', doc, docId: doc?._id, modifiedFields});
            return;
        }
        ...
        advancedDebug({log: 'ObservableCollection changed values', doc, docId: doc?._id, modifiedFields,projectedNew,projectedOld, changed});
