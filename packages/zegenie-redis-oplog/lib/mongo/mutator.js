...
export default class Mutator {
  static insert(Originals, data, _config) {
    ...
    try {
      ...
      if (!config.skipSave) this.setCache(doc);

      // the callback is not handled in the insert above
      if (typeof _config === 'function')
        runCallbackInBackground(() => _config.call(this, null, docId));

      if (
        !this._skipRedis &&
        config.pushToRedis &&
        (!this._weakDiffFields ||
          _.difference(_.keys(doc), this._weakDiffFields).length)
      ) {
      ...
      if (typeof _config === 'function')
        runCallbackInBackground(() => _config.call(this, e));
      throw e;
    }
  }
  ...
  static update(Originals, selector, modifier, _config, callback) {
    ...
    if (config.upsert)
      return Mutator.handleUpsert.call(
        this,
        Originals,
        selector,
        modifier,
    ...
    try {
      ...
      const result = config.skipDB
        ? 0
        : Originals.update.call(this, updateSelector, modifier, config);
      ...
      if (this._skipDiff || config.skipDiff) {
        ...
        docs.forEach((doc, index) => {
          ...
          if (!config.skipSave || this.hasCache(doc._id)) this.setCache(doc);
          docs[index] = _.pick(doc, pickFields);
        });
      } else {
        docs.forEach((before, index) => {
          ...
          if (diff) {
            if (!config.skipSave || this.hasCache(after._id))
              this.setCache(after);
      ...
      if (callback)
        runCallbackInBackground(() => callback.call(this, null, result));

      if (this._skipRedis || !config.pushToRedis || docs.length == 0)
        return result;

      if (
        !this._weakDiffFields ||
        _.difference(topLevelFields, this._weakDiffFields).length
      ) {
        dispatchUpdate(
          config.optimistic,
          this._name,
  ...
  static handleUpsert(
    ...
    try {
      ...
      if (result.insertedId) {
        ...
        // prone to race conditions in secondary-reads environment
        else {
      ...
      // updated
        ...
        if (docIds.length === 0 || result.numberAffected !== docIds.length) {
          ...
          if (this._skipDiff || config.skipDiff) {
            ...
            docs.forEach((doc, index) => {
              LocalCollection._modify(doc, modifier);
              if (!config.skipSave || this.hasCache(doc._id))
                this.setCache(doc);
              docs[index] = _.pick(doc, pickFields);
            });
          } else {
  ...
  static remove(Originals, selector, _config) {
    ...
    if (docs.length === 0) {
      if (typeof _config === 'function')
        runCallbackInBackground(() => _config.call(this, null, 0));
      return 0;
    }
    ...
    try {
      ...
      // we run the callback after we call the DB in case there is an error
      if (typeof _config === 'function')
        runCallbackInBackground(() => _config.call(this, null, result));
