...
const DeepDiff = (function () {
  ...
  function deepDiff(lhs, rhs, changes, path, key, stack) {
    ...
    if (typeof key !== 'undefined' && key != null) currentPath.push(key);

    const ltype = realTypeOf(lhs);
    const rtype = realTypeOf(rhs);
    ...
    const ldefined = ltype != 'null';
    const rdefined = rtype != 'null';
    var i, j, k, other;
    ...
    if (!ldefined && rdefined) {
      ...
      if (!other) {
        ...
        if (ltype == 'array') {
          ...
          // this for loop runs on remaining pkeys we have not processed yet in the for loop above
