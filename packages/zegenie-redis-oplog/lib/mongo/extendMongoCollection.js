...
import { setFieldsToIgnore as setRaceFieldsToIgnoreCore } from '../redis/raceDetectionManager';
import { isIdSelector, getIdsFromSelector } from './lib/idSelector';
import { addToWatch, removeFromWatch } from '../redis/watchManager';

const DEBUG_CACHE = (Meteor.isDevelopment || Meteor.isStaging) && false;
...
export default () => {
  ...
  _.extend(proto, {
     ...
     * @returns {*}
     */
    upsert(selector, modifier, config, callback) {
      if (!config) config = { upsert: true };
      else config.upsert = true;
      return Mutator.update.call(
        this,
        Originals,
        selector,
    ...
    addSub(id) {
      ...
      DEBUG_CACHE &&
        console.log(
          'addSub ' + this._name + ': ' + id + ' (' + this._subs[id] + ')'
        );
    },
    ...
    removeSub(id) {
      ...
      if (this._subs[id]) {
        ...
        DEBUG_CACHE &&
          console.log(
            'removeSub ' + this._name + ': ' + id + ' (' + this._subs[id] + ')'
          );
      }
    },
    ...
    startCaching(timeout) {
      ...
      const findOne = (selector = {}, options = {}) => {
        ...
        else if (_.isObject(selector) && typeof selector._id == 'string')
          id = selector._id;
        // first we check cache for existing doc
        if (id) {
        ...
        // we failed to get from cache --> onto mongo
        if (!doc) {
          doc = Originals.findOne.call(
            this,
            selector,
            options.skipSave ? options : _.omit(options, 'fields')
          );
          if (!doc) return undefined;
          DEBUG_CACHE &&
            console.log(
              'findOne: Fetched from DB ' + this._name + ' ' + doc._id
            );
          ++this._misses;
        ...
        if (options?.fields) {
          try {
            const projector = LocalCollection._compileProjection(
              options.fields
            );
      ...
      const find = (selector = {}, options = {}) => {
        ...
        const cursor = Originals.find.call(
          this,
          selector,
          options.skipSave ? options : _.omit(options, 'fields')
        );
        ...
        if (isIdSelector(selector)) {
          ...
          cursor.map = (cb, _this) =>
            this.fetchInCacheFirst(ids, options).map(
              _this ? cb.bind(_this) : cb
            );
          cursor.forEach = (cb, _this) =>
            this.fetchInCacheFirst(ids, options).forEach(
              _this ? cb.bind(_this) : cb
            );
        ...
        // we could do some optimizations here (for instance, edge cases where the cache satisfies our needs) but doesn't seem worthwhile at the moment
          ...
          const projector = options.fields
            ? LocalCollection._compileProjection(options.fields)
            : null;
          var overrideCB;
          if (options.skipSave)
            overrideCB = (cb, _this) => {
            ...
            overrideCB = (cb, _this) => {
              ...
              return (doc, index, __this) => {
                ...
                // any doc we touch, we have to save it
                  ...
                  DEBUG_CACHE &&
                    console.log(
                      'find: Fetched from DB ' + this._name + ' ' + doc._id
                    );
                  this.setCache(doc);
                }
                if (projector) {
          ...
          cursor.map = (cb, _this) =>
            origMap.call(cursor, overrideCB(cb, _this));

          // forEach
          const origForEach = cursor.forEach;
          cursor.forEach = (cb, _this) =>
            origForEach.call(cursor, overrideCB(cb, _this));

          // fetch
          const origFetch = cursor.fetch;
          cursor.fetch = (cb, _this) =>
            origFetch.call(cursor, overrideCB(cb, _this));

          // count -- we will make the assumption that we don't really need the docs as we are not processing them anywhere
      ...
      this.direct = {
        ...
        update: (selector, modifier, config, callback) =>
          proto.update.call(this, selector, modifier, config, callback),
        remove: (selector, config) => proto.remove.call(this, selector, config),
        upsert: (selector, modifier, config, callback) =>
          proto.upsert.call(this, selector, modifier, config, callback),
      };
      ...
      this.before = {
        find: (func) => {
          if (!this._beforeFindHooks) {
            ...
            this.find = (selector, options) => {
              this._beforeFindHooks.forEach((func) =>
                func.apply(null, selector, options)
              );
              const result = this.direct.find(selector, options);
              this._afterFindHooks.forEach((func) =>
                func.apply(null, selector, options)
              );
              return result;
            };
        ...
        insert: (func) => {
          if (!this._beforeInsertHooks) {
            ...
            this.insert = (data, config) => {
              ...
              this._beforeInsertHooks.forEach((func) =>
                func.call(null, userId, data)
              );
              const _id = this.direct.insert(data, config);
              data._id = _id;
              this._afterInsertHooks.forEach((func) =>
                func.call({ _id }, userId, data)
              );
              return _id;
            };
          }
        ...
        update: (func) => {
          if (!this._beforeUpdateHooks) {
            ...
            this.update = (selector, modifier, config, callback) => {
              const docs = config?.multi
                ? this.direct.find(selector, config).fetch()
                : [this.direct.findOne(selector)];
              if (docs.length == 0 || docs[0] == null) return;
              const userId = getUserId();
              const fields = getFields(modifier);
              this._beforeUpdateHooks.forEach((func) =>
                docs.forEach((doc) =>
                  func.call(null, userId, doc, fields, modifier, config)
                )
              );
              const result = this.direct.update(
                selector,
                modifier,
                config,
                callback
              ...
              docs.forEach((previous) => {
                ...
                this._afterUpdateHooks.forEach((func) =>
                  func.call(
                    { previous },
                    userId,
                    after,
        ...
        upsert: (func) => {
          if (!this._beforeUpsertHooks) {
            this._beforeUpsertHooks = [];
            this._afterUsertHooks = [];
            this.upsert = (selector, modifier, config, callback) => {
              const docs = config?.multi
                ? this.direct.find(selector, config).fetch()
                : [this.direct.findOne(selector)];
              if (docs.length == 0 || docs[0] == null) return;
              const userId = getUserId();
              const fields = getFields(modifier);
              this._beforeUpsertHooks.forEach((func) =>
                docs.forEach((doc) =>
                  func.call(null, userId, doc, fields, modifier, config)
                )
              );
              const result = this.direct.upsert(
                selector,
                modifier,
                config,
                callback
              ...
              docs.forEach((previous) => {
                const after = this.direct.findOne(previous._id);
                this._afterUsertHooks.forEach((func) =>
                  func.call(
                    { previous },
                    userId,
                    after,
          ...
          this._beforeUpsertHooks.push(func);
        },
        remove: (func) => {
          if (!this._beforeRemoveHooks) {
            ...
            this.remove = (selector, config) => {
              ...
              this._beforeRemoveHooks.forEach((func) =>
                docs.forEach((doc) => func.call(null, userId, doc, config))
              );
              const result = this.direct.remove(selector, config);
              this._afterRemoveHooks.forEach((func) =>
                docs.forEach((doc) => func.call(null, userId, doc, config))
              );
              return result;
            };
        ...
        upsert: (func) => {
          if (!this._beforeUpsertHooks) {
            ...
            this.upsert = (selector, modifier, config, callback) => {
              ...
              docs.forEach((previous) => {
                const after = this.direct.findOne(previous._id);
                this._afterUpsetHooks.forEach((func) =>
                  func.call(
                    { previous },
                    userId,
                    after,
          ...
          this._afterUpsetHooks.push(func);
        },
        remove: (func) => {
          if (!this._beforeRemoveHooks) {
            this._beforeRemoveHooks = [];
    ...
    fetchInCacheFirst(ids, options = {}) {
      if (this._isNotCached)
        return Originals.find
          .call(this, { _id: { $in: ids } }, options)
          .fetch();
      var result = [];
      const notFoundIds = [];
      ids.forEach((id) => {
      ...
      if (notFoundIds.length) {
        ...
        DEBUG_CACHE &&
          console.log(
            'fetchInCacheFirst: Fetched from DB ' +
              this._name +
              ' ' +
              notFoundIds.join(',')
          );
    ...
    setReadPreference(value = 'primary') {
      this._readPreference = value;
    },

    setRaceFieldsToIgnore(fields) {
      setRaceFieldsToIgnoreCore(this._name, fields);
    },

    setWeakDiffFields(fields) {
      this._weakDiffFields = fields;
    },
...
Meteor.methods({
  'RedisOplog.getCache': (key, collectionName, id) => {
    if (!Config.key)
      throw new Meteor.Error('No key provided in redis-oplog config');
