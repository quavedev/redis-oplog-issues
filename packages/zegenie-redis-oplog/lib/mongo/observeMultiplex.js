...
export function ObserveMultiplexer(options) {
  ...
  if (options.fields)
    this.projector = LocalCollection._compileProjection(options.fields);

  CALLBACKS.forEach(function (name) {
    self[name] = function (/* ... */) {
      self._applyCallback(name, _.toArray(arguments));
    };
  });
}
...
_.extend(ObserveMultiplexer.prototype, {
  addHandleAndSendInitialAdds: function (handle) {
    ...
    this._queue.runTask(function () {
      ...
      // Send out whatever adds we have so far (whether or not the multiplexer is ready)
  ...
  _applyCallback: function (callbackName, args) {
    ...
    this._queue[runType](function () {
      ...
      if (callbackName == 'added') {
        if (self._ids.includes(id))
          console.error('ObserveMultiplex - added: Doc present!', doc);
        else {
          self._ids.push(id);
        ...
        if (index != -1) {
          ...
          console.error(
            'ObserveMultiplex - removed: Doc ' + id + ' not found!'
          );
      } else if (callbackName == 'changed') {
        if (!self._ids.includes(id))
          console.error('ObserveMultiplex - changed: Doc not present!', doc);
      } else {
        console.error(
          'RedisOplog - ObserveMultiplex: Unknown callback ' + callbackName
        );
      }

      // If we haven't finished the initial adds, then we should only be getting adds
      if (!self._ready() && callbackName !== 'added')
        throw new Error(
          'RedisOplog - ObserveMultiplex: Got ' +
            callbackName +
            ' during initial adds'
        );

      if (doc && modifiedFields?.length)
        doc = _.pick(doc, [...modifiedFields, '_id']);

      if (self.projector && doc) {
        try {
          doc = self.projector(doc);
        } catch (e) {}
      }
...
export function ObserveHandle(multiplexer, callbacks) {
  ...
  CALLBACKS.forEach(function (name) {
    if (callbacks[name]) self['_' + name] = callbacks[name];
  });
  self._stopped = false;
