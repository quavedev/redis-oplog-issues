...
class RedisSubscriptionManager {
  ...
  getAllRedisSubscribers() {
    ...
    for (var channel in this.subscribers) {
      this.subscribers[channel].forEach((_redisSubscriber) =>
  ...
  detach(redisSubscriber) {
    this.queue.queueTask(() => {
      _.each(redisSubscriber.channels, (channel) => {
        ...
        this.subscribers[channel] = _.without(
          this.subscribers[channel],
          redisSubscriber
        );
        if (this.subscribers[channel].length === 0 && !this.watchers[channel])
  ...
  stopWatch(channel) {
    if (!this.subscribers[channel] || this.subscribers[channel].length == 0)
      this.destroyChannel(channel);
  }
  ...
  process(channel, data, fromRedis) {
    ...
    debug(
      `[RedisSubscriptionManager] Received event: "${data[RedisPipe.EVENT]}" to "${channel}"`
    );

    const collectionName = channel.split('::').shift();
    const collection = hasSubs
      ? subscribers[0].observableCollection.collection
      : Mongo.Collection.__getCollectionByName(collectionName);
    ...
    if (hasSubs) {
      if (fromRedis) {
        subscribers.forEach((redisSubscriber) => {
          try {
            redisSubscriber.process(event, doc, fields);
          } catch (e) {
            debug(
...
const redisSubscriptionManager = new RedisSubscriptionManager();
