...
export const onMutation = (collectionName, _id, fields, doc) => {
  ...
  if (item) {
    if (item.timer) {
      debug(
        '[RaceDetectionManager - onMutation] Skipping ' +
          uid +
          ' as we are waiting for forced update'
      );
    }
    // it's too far out - we are ok
    else if (now - item.updatedAt > TIMEOUT)
      history[uid] = {
        collectionName,
        _id,
        fields,
    ...
    // check for fields collision
      ...
      if (
        common.length &&
        DeepDiff(_.pick(item.doc, common), _.pick(doc, common))
      ) {
        console.log(
          'Redis-Oplog: Handling potential race condition on ' +
            uid +
            ': ' +
            common.join(', ')
        );
...
export const onTimer = (uid) => {
  ...
  if (item) {
    ...
    if (!collection) {
      ...
      if (driver && collection._readPreference !== 'primary')
        collection = new Mongo.Collection(item.collectionName, {
          _driver: driver,
...
export const init = () => {
  ...
  if (Config.secondaryReads) {
    const mongoURL =
      process.env.MONGO_URL.replace(/readPreference=[^&]+\&?/, '') +
  ...
  skip = false;
  // don't need Meteor.setInterval as we are not calling any fiber-related funcgtions
  setInterval(cleanupHistory, 10 * TIMEOUT);
};

export const setFieldsToIgnore = (collectionName, fields) => {
  debug(
    '[RaceDetectionManager - setFieldsToIgnore]: ' +
      collectionName +
      ' ' +
      fields.join(', ')
  );
...
if (Meteor.isDevelopment) {
  Meteor.methods({
    'redisOplog.onMutation': onMutation,
  });
}
