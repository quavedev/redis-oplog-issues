...
import { init as initializeRaceDetectionManager } from './redis/raceDetectionManager';
...
export default (config = {}) => {
  if (isInitialized)
    throw new Meteor.Error(
      'RedisOplog: You cannot initialize RedisOplog twice'
    );
  ...
  if (Config.oplogToRedis) {
    if (Config.mutationDefaults.pushToRedis)
      console.error('RedisOplog: Both pushToRedis & oplogToRedis are true!');
    Config.redisPrefix =
      process.env.MONGO_URL.split('/').pop().replace(/\?.*/, '') + '.';
    console.log(
      'RedisOplog: OplogToRedis configured with db name "' +
        Config.redisPrefix +
        '"'
    );
  }
  ...
  getRedisListener({
    onConnect() {
      ...
      RedisSubscriptionManager.getAllRedisSubscribers().forEach(
        (redisSubscriber) => reload(redisSubscriber.observableCollection)
      );
    },
  });
  ...
  if (
    Config.secondaryReads === null &&
    /readPreference=secondary/.test(process.env.MONGO_URL)
  )
    Config.secondaryReads = true;
  if (Config.secondaryReads)
    console.log('RedisOplog: Reading from secondary DB nodes');

  if (Config.raceDetection !== false) initializeRaceDetectionManager();

  // wasteful events
  Meteor.startup(function () {
    const loginService = Mongo.Collection.__getCollectionByName(
  ...
  Meteor.methods({
    __getCollectionStats: function (collectionName) {
      ...
      return {
        ...
        hitRatio: collection._hits
          ? (100 * collection._hits) / (collection._hits + collection._misses)
          : 0,
      };
    },
  });
