...
export default class ObservableCollection {
  ...
  init() {
    ...
    this.collection
      .find(this.selector, this.options)
  ...
  addById(docId) {
    ...
    if (doc) {
      this.ids.set(docId, doc.updatedAt || new Date());
  ...
  compareWith(rightMap, callbacks) {
    ...
    leftMap.forEach((leftUpdatedAt, key) => {
      ...
      if (rightValue != null) {
        if (callbacks.both) {
          if (!rightValue.updatedAt || leftUpdatedAt - rightValue.updatedAt < 0)
            callbacks.both(rightValue);
        }
      } else callbacks.leftOnly && callbacks.leftOnly(key);
    });
